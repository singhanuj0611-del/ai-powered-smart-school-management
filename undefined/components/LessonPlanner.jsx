...
const LessonPlanner = ({ settings, onRequireKey }) => {
  ...
  const generateDummyPlan = () => {
    return {
      objectives: [
        `Students will understand the core concepts of ${form.topic}.`,
        `Students will connect ${form.topic.toLowerCase()} to real-world applications.`,
        `Students will engage in collaborative exploration and critical thinking.`
      ],
      materials: [
      ...
      activities: [
        `Warm-up: Quick poll about ${form.topic.toLowerCase()} to gauge prior knowledge.`,
        `Main activity: Guided exploration with hands-on examples and group discussions.`,
        `Wrap-up: Students create a brief summary and share key takeaways.`
      ],
      assessment: [
        "Formative: Exit ticket with one question and one insight.",
        "Summative: Short quiz covering main concepts.",
        "Reflection: Student self-assessment using provided rubric."
      ]
    };
  };
  ...
  const generatePlan = async (event) => {
    ...
    if (!settings?.aiKey && !settings?.geminiKey) {
      setError("Add at least one API key (OpenAI or Gemini) in Settings to generate lesson plans.");
      onRequireKey?.();
      return;
    }
    ...
    setIsThinking(true);
    try {
      const parsedPlan = await generateAIJSON({
        ...
        dummyDataGenerator: generateDummyPlan
      });

      setPlan(parsedPlan);
      setSource(parsedPlan._source || "openai");
    } catch (caughtError) {
      setError(
        caughtError?.message?.includes("JSON")
          ? "EduAI responded in an unexpected format. Try again."
          : "We couldn't reach EduAI. Using fallback data."
      );
        ...
        >
          <div>
            ...
            <p style={{ margin: "6px 0 0", opacity: 0.65 }}>
              {source === "dummy" 
                ? "Sample lesson plan (add API keys for AI-generated content)."
                : source === "gemini"
                ? "Crafted by Google Gemini AI."
                : source === "openai"
                ? "Crafted in seconds by OpenAI."
